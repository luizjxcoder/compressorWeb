...
function App() {
     ...
     const resetConversion = useCallback(() => {
          // Resetar apenas os dados de conversão, mantendo as imagens carregadas
          setImages((prev) => prev.map((img) => ({
               ...img,
          ...
          toast.success('Conversão resetada - Imagens prontas para nova conversão', {
               description: 'Dados de compressão limpos, imagens mantidas'
          });
     }, []);
                                   ...
                                   <div className="flex items-center space-x-4 text-sm">
                                        {Object.entries(formatCounts).map(([format, count]) =>
                                             <div key={format} className="flex items-center space-x-2">
                                                  <div className={`w-3 h-3 rounded-full ${format === 'WEBP' ? 'bg-green-400' :
                         ...
                         <div className="flex flex-col space-y-4">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                                   {Object.entries(webPresets).map(([key, preset]) => {
                                        ...
                                        return (
                                             <button
                                                  ...
                                                  className={`p-4 rounded-lg border transition-all duration-200 ${compressionOptions.webMode === key ?
                                                  ...
                                                  <div className="flex justify-center mb-3">
                                                       <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${compressionOptions.webMode === key ?
                                                                 'bg-blue-500/20' :
                                                                 'bg-slate-600/50'}`
                                                       }>
                                                            <IconComponent className={`w-6 h-6 ${compressionOptions.webMode === key ?
                                                                      'text-blue-400' :
                                                                      'text-slate-400'}`
                                                            } />
                                                       </div>
                                   ...
                                   <button
                                        ...
                                        className={`p-4 rounded-lg border transition-all duration-200 ${compressionOptions.webMode === 'custom' ?
                                                  'bg-purple-600/20 border-purple-500 text-purple-300 shadow-lg shadow-purple-500/20' :
                                                  'bg-slate-700/50 border-slate-600 text-slate-300 hover:bg-slate-600/50 hover:border-slate-500'}`
                                        }>

                                        <div className="flex justify-center mb-3">
                                             <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${compressionOptions.webMode === 'custom' ?
                                                       'bg-purple-500/20' :
                                                       'bg-slate-600/50'}`
                                             }>
                                                  <Settings className={`w-6 h-6 ${compressionOptions.webMode === 'custom' ?
                                                            'text-purple-400' :
                                                            'text-slate-400'}`
                                                  } />
                                             </div>
               ...
               <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    {images.length === 0 ?
                         ...
                         <div className="space-y-6">
                              {/* Seção de Processamento e Conversão */}
                              <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700/50">
                                   ...
                                   {/* Botões de Ação - Movidos para dentro da seção */}
                                   {images.length > 0 &&
                                        <div className="flex flex-wrap gap-3 justify-center">
                                             ...
                                             {Object.entries(formatCounts).map(([format, count]) =>
                                                  <button
                                                       ...
                                                       disabled={count === 0}
                                                       className="flex items-center space-x-2 px-3 py-2 hover:bg-green-700 disabled:bg-slate-600 disabled:cursor-not-allowed rounded-lg transition-colors text-sm bg-[#A5B4FC] text-[#F1F5F9]">
                                             ...
                                             <button
                                                  ...
                                                  className="flex items-center space-x-2 px-4 py-2 hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed rounded-lg transition-colors bg-[#818CF8] text-[#E2E8F0]">
                                             ...
                                             <button
                                                  ...
                                                  className="flex items-center space-x-2 px-4 py-2 hover:bg-orange-700 rounded-lg transition-colors bg-[#4F46E5] text-[#E2E8F0]">
                                             ...
                                             <button
                                                  onClick={clearAll}
                                                  className="flex items-center space-x-2 px-4 py-2 hover:bg-red-700 rounded-lg transition-colors bg-[#312E81] text-[#E2E8F0]">
